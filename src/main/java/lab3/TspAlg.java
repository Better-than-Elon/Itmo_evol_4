...
public class TspAlg {
    public TspParser tspParser;
    static int populationSize = 10; // size of population
    static int generations = 100000; // number of generations

    public static void main(String[] args) {
        String problem = "xqg237.tsp"; // name of problem or path to input file

        TspParser tspParser = new TspParser(problem);

        Random random = new Random(); // random

        CandidateFactory<TspSolution> factory = new TspFactory(tspParser.getDimention()); // generation of solutions
...
//        SelectionStrategy<Object> selection = new RankSelection();//RouletteWheelSelection(); // Selection operator
        SelectionStrategy<Object> selection = new TournamentSelection(new Probability(0.95));

        FitnessEvaluator<TspSolution> evaluator = new TspFitnessFunction(tspParser.getCities()); // Fitness function
